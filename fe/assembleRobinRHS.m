function H = assembleRobinRHS(model)

% Assembles Robin right-hand-side vector
% SYNTAX:
%   H = assembleRobinRHS(model)
% 
% DESCRIPTION:
% H = assembleRobinRHS(model) assembles Robin right-hand-side vector
%
...
%
% 
% OUTPUTS: - G: Robin right-hand-side vector
%
% 
...
% CHANGELOG: --
H = zeros(length(model.nodes),1);
if isa(model.h,'function_handle')
    for i=1:size(model.rnodes,1)
        a = model.nodes(model.rnodes(i));
        H(model.rnodes(i)) = model.h(a);
    end
else
    for i=1:size(model.rnodes,1)
        H(model.rnodes(i)) = model.h;
    end
end
